...
<<<<<<< HEAD
    ...
    const currentStatus = booking.status ? booking.status.toLowerCase() : '';
    if (currentStatus === 'expired' || currentStatus === 'rejected' || currentStatus === 'declined') {
      return res.status(400).json({ error: "Booking is not eligible for payment" });
    }

    if (currentStatus !== 'accepted') {
      return res.status(400).json({ error: "Payment allowed only after booking acceptance" });
    }
